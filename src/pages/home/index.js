...
const Home = ({data, fetchOwnerProductProperties, practiceType, startDate, endDate, token, filterData, isFetching, count = 0,updateFilerData, selState, selCounty}) => {
    ...
    const handleChangePage = (event, newPage) => {
        fetchOwnerProductProperties({currentPage: newPage, from: startDate, to: endDate, token, perPage: rowsPerPage, practiceType, filters:JSON.stringify(filterData), count, state: selState, county: selCounty})
        setPage(newPage)
    };
    const handleChangeRowsPerPage = (event) => {
        const perPage =parseInt(event.target.value, 10)
            fetchOwnerProductProperties({currentPage: page, from: startDate, to: endDate, token,perPage, practiceType, filters:JSON.stringify(filterData), count, state: selState, county: selCounty})
    ...
    const onAddFilter = () => {
        ...
        fetchOwnerProductProperties({currentPage: page, from: startDate, to: endDate, token, perPage: rowsPerPage, practiceType, filters:JSON.stringify(newTags), count: -1, state: selState, county: selCounty})
