...
// const GRAPHQL_ENDPOINT = "https://www.homesalesclub-api.com/graphql"
...
export const fetchOwnerProductProperties = (data) => apolloClient.query({
    query: gql `
    query FetchOwnerProductProperties($token: String!, $from: String!, $to: String!,$perPage: Int,$currentPage: Int, $practiceType: String, $filters:String, $count: Int!, $state: String, $county: String,$zip:String) {
        fetchOwnerProductProperties(token: $token, from: $from, to: $to,currentPage:$currentPage,perPage:$perPage,practiceType:$practiceType,filters:$filters,count: $count, state: $state, county: $county,zip:$zip) {
            success
            data
            count
            error
  ...
  `,
    variables: {
        ...
        state: data.state ? data.state : 'all',
        county: data.county ? data.county : 'all',
